...
const Index = () => {
  ...
  const whyCards = why.cards?.length ? why.cards : DEFAULT_WHY_CARDS;

  const websiteSchema = {
    "@context": "https://schema.org", "@type": "WebSite", "name": "Cairo By Nights Restaurant & Bar", "url": "https://www.caironightsrestaurant.com.au",
    "potentialAction": { "@type": "SearchAction", "target": { "@type": "EntryPoint", "urlTemplate": "https://www.caironightsrestaurant.com.au/menu?search={search_term_string}" }, "query-input": "required name=search_term_string" }
  };

  const restaurantSchema = {
    "@context": "https://schema.org", "@type": ["Restaurant", "LocalBusiness"], "name": "Cairo By Nights Restaurant & Bar",
    ...
    "knowsAbout": ["Egyptian cuisine", "Halal food", "Belly dancing", "Shisha lounge", "Middle Eastern entertainment", "Traditional Arabic music", "Egyptian hospitality"],
    "amenityFeature": [
      { "@type": "LocationFeatureSpecification", "name": "Live Belly Dancing", "value": "Professional belly dancing shows every Friday, Saturday, and Sunday at 9PM" },
      { "@type": "LocationFeatureSpecification", "name": "Premium Shisha Lounge", "value": "Authentic Egyptian shisha with premium tobacco blends" },
      { "@type": "LocationFeatureSpecification", "name": "Live Arabic Music", "value": "Traditional Arabic music performances on Friday and Sunday evenings" },
      { "@type": "LocationFeatureSpecification", "name": "Private Function Room", "value": "Private event space available for birthdays, corporate events, and celebrations" },
      { "@type": "LocationFeatureSpecification", "name": "Halal Certified", "value": "All meat is halal and kitchen follows strict halal guidelines" }
    ]
  };
    ...
    <div className="min-h-screen bg-background scroll-smooth">
      ...
      <main>
        ...
        <section className="section-padding bg-background">
          <div className="max-w-6xl mx-auto">
            <div className="max-w-4xl mx-auto text-center mb-12 animate-fade-in">
              <h2 className="text-3xl md:text-4xl font-playfair font-bold mb-6 egyptian-gold">{welcome.heading || 'Welcome to Cairo By Nights'}</h2>
              <p className="text-lg text-muted-foreground leading-relaxed">{welcome.subheading || "Melbourne's Premier Egyptian Restaurant Experience"}</p>
            </div>
        ...
        <section className="section-padding bg-secondary/20">
          <div className="max-w-6xl mx-auto">
            <div className="max-w-4xl mx-auto text-center mb-12 animate-fade-in">
              <h2 className="text-3xl md:text-4xl font-playfair font-bold mb-6 egyptian-gold">{why.heading || 'Why Choose Cairo By Nights'}</h2>
              <p className="text-lg text-muted-foreground leading-relaxed">{why.subheading || 'Experience the Difference of Authentic Egyptian Excellence'}</p>
            </div>

            <div className="grid md:grid-cols-2 gap-8 mb-8">
              {whyCards.map((card, i) => (
                <div key={i} className="bg-muted/50 p-6 rounded-lg">
                  <h3 className="text-xl font-playfair font-semibold mb-4 text-primary">{card.title}</h3>
            ...
            <div className="text-center">
              <p className="text-lg text-muted-foreground mb-6">
                <strong className="text-foreground">{why.bottomTextBold || 'Join 5,000+ satisfied guests'}</strong> {why.bottomTextRest || 'who have rated us 4.8/5 stars. Experience the Cairo By Nights difference today.'}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="section-padding bg-background">
          <div className="max-w-6xl mx-auto">
            <div className="max-w-4xl mx-auto text-center mb-12 animate-fade-in">
              <h2 className="text-3xl md:text-4xl font-playfair font-bold mb-6 egyptian-gold">{visit.heading || 'Visit Us in Carlton'}</h2>
              <p className="text-lg text-muted-foreground leading-relaxed">{visit.subheading || "Easy to Find on Lygon Street, Melbourne's Iconic Dining Precinct"}</p>
            </div>

            <div className="grid md:grid-cols-3 gap-8 mb-8">
              {(visit.cards || DEFAULT_VISIT_CARDS).map((card: any, i: number) => (
                <div key={i} className="text-center">
                  <div className="bg-primary/10 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                    <span className="text-3xl">{card.emoji}</span>
                  </div>
                  <h3 className="font-playfair font-semibold text-xl mb-3 text-primary">{card.title}</h3>
                  <p className="text-muted-foreground whitespace-pre-line">{card.body}</p>
                </div>
              ))}
            </div>
            ...
            <div className="bg-muted/50 p-8 rounded-lg text-center">
              <p className="text-lg text-muted-foreground mb-6">
                {visit.ctaBoxIntro || "First time visiting Carlton? Our comprehensive location guide includes detailed parking information, tram routes from across Melbourne, nearby attractions to explore before or after dinner, and everything you need to plan your visit to Lygon Street's premier Egyptian restaurant."}
              </p>
              <a href={visit.ctaBoxButton?.href || '/location'} className="inline-block px-8 py-3 bg-primary text-black font-semibold rounded-lg hover:bg-primary/90 transition-colors duration-300">
                {visit.ctaBoxButton?.label || 'View Complete Location Guide →'}
              </a>
            </div>
          </div>
        </section>
